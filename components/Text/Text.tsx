...
export const TextBlock = ({ children, align, gap }: { children: React.ReactNode, align?: "center", gap?: "small" | "none" }) => {
...
export const ArrowList = ({ items }: { items: React.ReactNode[] }) => {
    return <ul className={`${styles.list} ${styles.arrowList}`}>
        {items.map((item, i) => {
            return (
                <li key={i}>
                    <div className={styles.arrowListIndicator}>
                        <svg width="10" height="9" viewBox="0 0 10 9" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1 4.5H8M8 4.5L4.5 1M8 4.5L4.5 8" stroke="black" stroke-width="1.5" stroke-linecap="round" />
                        </svg>
                    </div>
                    {item}
                </li>
